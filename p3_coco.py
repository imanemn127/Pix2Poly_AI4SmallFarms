...
def _safe_to_tensor(arr):
    """Converts any NumPy array to a PyTorch FloatTensor, even with NumPy 2.x."""
    if isinstance(arr, torch.Tensor):
        return arr.float()
    # We bypass torch.from_numpy() by using a copy in memory via torch.FloatTensor
...
class P3Dataset(Dataset):
    ...
    def __getitem__pix2poly(self, index: int):
        ...
        # Ensure that the image is a FloatTensor (C, H, W)
        if not isinstance(image, torch.Tensor):
            image = _safe_to_tensor(image)
            if image.dim() == 3 and image.shape[-1] == 3:  # HWC -> CHW
